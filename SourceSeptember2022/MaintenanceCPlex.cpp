...
int Maintenance::IntegerFormulationFullGRB_Reactive(int tipo_pool)
{
	...
	try
	{
		...
		//Reparar sol
		if (M_Reparar == true || M_Best_I_T.size() != 0)
		{
			model.set(GRB_IntParam_Threads, 1);
			for (int i = 0; i < M_N_Interventions; i++)
			{
		...
		//		Max_Objectives = 12;
		double Total_Gurobi = M_Time_Populate;

//		for (int tipos_quantiles = 1; tipos_quantiles <= 2; tipos_quantiles++)
//		{

			int Max_Objectives = Numero_Objectives();
			LimpiarCoeficientes();
			M_Tipo_Quantiles = 2 ;
			double Tiempo_Total_Gurobi = Total_Gurobi / 1;
			M_Time_Populate = M_Time_Populate / Max_Objectives;
			Max_Objectives =
			    (double)(2 * Max_Objectives) * (double)(M_Total_Time / (double)900);
		        double GAP = 0.0005;

			if (M_Total_Time > 3000)
		        {
			        Max_Objectives = Max_Objectives / 2;
			        GAP = 0.0001;
		        }
			if (Max_Objectives <= 8)
				M_Time_Populate = 50;
				...
				M_Time_Populate = 25;
			Max_Objectives = Tiempo_Total_Gurobi / M_Time_Populate;
			Tiempo_Total_Gurobi = Max_Objectives * M_Time_Populate;
			...
			model.set(GRB_IntParam_OutputFlag, 0);
			model.set(GRB_IntParam_Presolve,
			          1); // 1 - conservative, 0 - off, 2 - aggressive
			model.set(GRB_IntParam_PrePasses,
			          1); // do not spend too much time in pre-solve
			model.set(GRB_IntParam_OutputFlag, 1);
			...
			Numero_sol = Numero_sol * Max_Objectives;
			//			M_Time_Populate = M_Time_Populate * Max_Objectives;
			if (M_Best_I_T.size() == 0 || M_Reparar)
				Max_Objectives = 1;
			...
			double time = 0;
			ftime(&time_ini);
			ModificarCoeficientes();
			double Best_sol = 2 * M_Best_fo + 1;
			double Best_Sol_Iter = 2 * M_Best_fo;
			int iter = -1;
			bool cambio = false;
			int iter2 = 0;
			int kiter = 0;
		        Alpha = 0;
			for (; kiter < Max_Objectives && time < Tiempo_Total_Gurobi; kiter++)
			//		while (true)
			{

				if (Best_Sol_Iter < (Best_sol - 0.01))
					iter = 0;
				else
					iter++;
				Best_sol = Best_Sol_Iter;
				if (iter > 1) // if no improvement in more than 2 iterations then decrease the penalties
				{
				        if (iter==2) 
						{
					        printf("Generar_Soluciones Iter2 %d\n", iter2);
					        model.set(GRB_IntParam_PoolSolutions,
					                  5 * Max_Objectives);
					        model.set(GRB_DoubleParam_PoolGap, GAP * 20);
					        model.set(GRB_IntParam_PoolSearchMode, 1);
				                ...
				                else
				                {
					                ...
					                iter2++;
					                /* if (iter2 == 2)
					        M_Tipo_Coeficientes = 2;
				        if (iter2 == 4)
					        M_Tipo_Coeficientes = 3;*/
					                if (iter2 == 2)
					                {
						                LimpiarCoeficientes();
						                M_Tipo_Coeficientes = 2;
						                iter = 0;
										Best_sol = 2 * M_Best_fo + 1;
						                Best_Sol_Iter = 2 * M_Best_fo;

					                }
					                else
					                {
						                //double reduce = 0.2 + ((rand_multithread() % 11) * 0.01);
						                iter = 0;
						                for (int t = 0; t < M_T; t++)
							                for (int s = 0;
							                     s <
							                     M_Scenarios_number[t];
							                     s++)
							                {
								                M_PenalScen[t][s] =
								                    reduce *
				...
				GRBLinExpr obj = 0;
				if ((kiter>=(Max_Objectives-3)) || (time + 2 * M_Time_Populate > Tiempo_Total_Gurobi) ||
				    (Tiempo_Total_Gurobi - time < 50) || (iter2>=2))
				{
					printf("Generar_Soluciones Iter2 %d\n", iter2);
					model.set(GRB_IntParam_PoolSolutions, 5 * Max_Objectives);
					model.set(GRB_DoubleParam_PoolGap, GAP * 100);
					model.set(GRB_IntParam_PoolSearchMode, 1);
				}
				if (((time + M_Time_Populate) > Tiempo_Total_Gurobi))
					M_Time_Populate = (Tiempo_Total_Gurobi - time) + 2;
					...
					if (key[i][j] >= 0)
						obj += X[key[i][j]] *
						       ((M_Interventions[i].Get_RiskQuantile(j) *
						         (1 - Beta)) +
						         ((Alpha)*M_Interventions[i].Get_RiskC(j)) +
						         ((1-Alpha)*M_Interventions[i].Get_RiskExcess(j))) 
						    ;

					*/
						if (key[i][j] >= 0)
						{
							//						nuevo = CoeficienteRiesgo(i, j);
							//						if (iter2==4)
							//						nuevo = CoeficienteRiesgoQuitando(i, j);
							//						else
							nuevo = CoeficienteRiesgo(i, j);

							/* antiguo = M_Interventions[i].Get_RiskC(j);
						if (nuevo < antiguo-0.1 || nuevo>antiguo+0.1)
						{
							int kk = 8;
						}*/
							//						obj += X[key[i][j]] * nuevo * 0.5;
							//						if (iter2 !=3)
						        obj += X[key[i][j]] * (nuevo)  ;
							//						else
							//							obj += X[key[i][j]] *
							//							       (nuevo + M_Interventions[i].Get_RiskExcess(j));
						}

						/* if (kiter > 0 && M_Best_I_T[i] == j &&
					    (M_Interventions[i].Get_RiskC(j) >
					     (M_Interventions[i].Get_Min_Risk() + 1)) &&
					    (M_Interventions[i].Get_RiskQuantile(j) >
					     (M_Interventions[i].Get_Min_RiskQ() + 1)) &&
					    get_random(0, 10) < 4)
						X[key[i][j]].set(GRB_DoubleAttr_LB, 1);*/
					}
				}
				...
				{
					M_Time_Populate += 15;
					//				Tiempo_Total_Gurobi += 15;
					//				if (Tiempo_Total_Gurobi > (M_Total_Time/2))
					//					Tiempo_Total_Gurobi = (M_Total_Time / 2);
				}
				int ibest_sol = 0;
				double best_val = 0;
				/* if (i != 0)
			...
			{
				val = FuncionObjetivoPenalizacion();
				ModificarCoeficientes();
			}*/
				...
				for (auto i = 0; i < Nsol; i++)
				{
					...
					{
						if (!M_Challenge_Mode) cout << "Iterativo_Iter " << iter << " Iter 2 " << iter2<< " Tipo Quantile "<< M_Tipo_Quantiles<< " ";
						val = FuncionObjetivoPenalizacion();
						ModificarCoeficientes();
					}
					if (val < Best_Sol_Iter)
						Best_Sol_Iter = val;
				...
				ftime(&t1);
				time = ((double)((t1.time - time_ini.time) * 1000 + t1.millitm -
				                 time_ini.millitm)) /
				       1000;
				if (!M_Challenge_Mode)
					printf("\nTiempo lleva Gurobi %.2f Tiempo total %.2f "
					       "Quantile %d \n",
					       time, Tiempo_Total_Gurobi, M_Tipo_Quantiles);
				if ((kiter == Max_Objectives - 1) &&
				    time < (Tiempo_Total_Gurobi - 10))
				{
					if (iter2 < (Max_Objectives / 3))
						Max_Objectives++;
				}
			}
//		}
...
void Maintenance::LimpiarCoeficientes()
{
	for (int i = 0; i < M_T; i++)
	{

		for (int s = 0; s < M_Scenarios_number[i]; s++) 
		{
			M_Coef[i][s] = 1.0;
			M_Freq[i][s] = 0;
			M_PenalScen[i][s] = 0;
...
void Maintenance::ModificarCoeficientes()
{
	for (int i = 0; i < M_T; i++)
	{

		M_Coef[i][0] = 1.0;
		if (M_Tipo_Coeficientes == 1)
		{
			if (M_Freq[i][0])
				M_Coef[i][0] += (M_PenalScen[i][0] / M_Freq[i][0]);
		}
		else
		{

			M_Coef[i][0] += (M_PenalScen[i][0]);

		}
		double sumcoef = M_Coef[i][0];
		for (int s = 1; s < M_Scenarios_number[i]; s++)
		{
			M_Coef[i][s] = 1.0;
			if (M_Tipo_Coeficientes == 1) {
			if (M_Freq[i][s])
				M_Coef[i][s] += (M_PenalScen[i][s] / M_Freq[i][s]);}
			else
			M_Coef[i][s] += (M_PenalScen[i][s]);
			sumcoef += M_Coef[i][s];
		}
		for (int s = 0; s < M_Scenarios_number[i]; s++)
			M_Coef[i][s] = M_Coef[i][s] / sumcoef;
	}
	for (int i = 0; i < M_N_Interventions; i++)
	{
		double sumaexceso = 0;
		for (int t = 0; t < M_T; t++) 
		{
			...
			M_CoefIT[i][t] = 1;
			if (M_FreqIT[i][t])
				sumaexceso += (M_PenalScenIT[i][t] / M_FreqIT[i][t]);
		}
		for (int t = 0; t < M_T; t++)
		{
			if (M_FreqIT[i][t])
				M_CoefIT[i][t] +=( (M_PenalScenIT[i][t] / M_FreqIT[i][t])/ sumaexceso);
		}
	}
...
double Maintenance::CoeficienteRiesgo(int intervention, int tinicio) 
{
	double meanrisk = 0;
	for (auto t = tinicio; t < tinicio + M_Interventions[intervention].Get_Delta(tinicio) ;t++) 
	{
		 double meanrisk_tini_t = 0;
		/* for (auto sc = 0; sc < M_Scenarios_number[t]; sc++) 
		{
			meanrisk_tini_t += M_Interventions[intervention].Get_RiskN(t, tinicio, sc);
		}
		meanrisk_tini_t = meanrisk_tini_t / M_Scenarios_number[t];*/
		for (auto sc = 0; sc < M_Scenarios_number[t]; sc++) 
		{

			meanrisk += M_Interventions[intervention].Get_RiskN(t, tinicio, sc)* M_Coef[t][sc];
//			if (M_PenalScen[t][sc]>1)
//			meanrisk += abs(M_Interventions[intervention].Get_RiskN(t, tinicio, sc) -
//			                meanrisk_tini_t) *M_Coef[t][sc];

		}
//		meanrisk+=meanrisk / M_Scenarios_number[t];
	}
	return meanrisk;

}
double Maintenance::CoeficienteRiesgoQuitando(int intervention, int tinicio)
{
	double meanrisk = 0;
	for (auto t = tinicio; t < tinicio + M_Interventions[intervention].Get_Delta(tinicio); t++)
	{
		for (auto sc = 0; sc < M_Scenarios_number[t]; sc++)
		{
			if (M_Scenario_Activo[t][sc])
			meanrisk += M_Interventions[intervention].Get_RiskN(t, tinicio, sc) ;
		}
		//		meanrisk+=meanrisk / M_Scenarios_number[t];
	}
	return meanrisk;
}
double Maintenance::CoeficienteRiesgoIT(int intervention, int tinicio)
{
	double meanrisk = 0;
	for (auto t = tinicio; t < tinicio + M_Interventions[intervention].Get_Delta(tinicio); t++)
	{
		...
		meanrisk +=
			    M_Interventions[intervention].Get_RiskC(t) * M_CoefIT[intervention][t];
...
//	M_Time_Populate = M_Time_Populate * 2;
	...
	try
	{
		...
		X = model.addVars(nmbvar, GRB_BINARY);
		M = model.addVars(M_T, GRB_CONTINUOUS);
		...
		if (M_Tipo_Formulation==3 )
		{
			for (int j = 0; j < M_T; j++)
			{
				...
				for (int l = 0; l < M_Scenarios_number[j]; l++)
				{
					...
					model.addConstr(expr <= M[j]+9e-6,
						"max_M" + to_string(j)+"_" + to_string(l));
		...
		if (M_Tipo_Formulation == 5)
		{
			for (int j = 0; j < M_T; j++)
			{
				...
				for (int l = 0; l < M_Scenarios_number[j]; l++)
				{
					...
					model.addConstr(expr >= m[j]+9e-6,
						"min_M" + to_string(j)+"_" + to_string(l));
...
//					expr += X[(*it)[1]][t2];
		...
		if (M_Reparar == true || M_Best_I_T.size() != 0 )
		{
			...
			for (int i = 0; i < M_N_Interventions; i++)
			{
				for (int j = 0; j < M_T; j++)
				{
					if (M_Best_I_T[i] == j) X[key[i][j]].set(GRB_DoubleAttr_Start, 1);
				}
			}
		}
		// OBJECTIVE
		...
		int NO_OBJECTIVE = false;
					...
					double Tiempo_Total_Gurobi = M_Time_Populate;
			
			if (M_Tipo_Formulation > 0)
			{
				Max_Objectives = 1;
				M_Time_Populate= Tiempo_Total_Gurobi * 2;
			}
			else
				M_Time_Populate = M_Time_Populate / (Max_Objectives + 1);
			...
			if (!M_Challenge_Mode)
				printf("\nTiempo %d Iteraciones %d Cuantas %d\n", M_Time_Populate,
				       M_Iter, Max_Objectives);
			double Cambio_fobj = 1 / (double)(Max_Objectives);
			int Numero_sol = M_Iter;
			if (M_Iter <= 2)
				Numero_sol = 3;
			Numero_sol = Numero_sol * 2;
...
//			M_Time_Populate = M_Time_Populate * Max_Objectives;
	...
	double time = 0;
	ftime(&time_ini);
			for (int kiter = 0; kiter < Max_Objectives && time<Tiempo_Total_Gurobi; kiter++)
			{
...
//					model.write("kk.lp");
				...
				if (M_Tipo_Formulation == 3)
					model.set(GRB_DoubleParam_NodefileStart, 3);
				if (M_Tipo_Formulation==0)
	...
	//			Alpha = 1;
				for (int j = 0; j < M_T; j++)
				{
			        ...
			        for (int i = 0; i < M_N_Interventions; i++)
			        {
					        ...
					        if (key[i][j] >= 0 )
								...
								{
									if (M_Tipo_Formulation == 1)
									{
										obj += X[key[i][j]] *
											(Alpha) * (M_Interventions[i].Get_RiskC(
													j));
				...
				model.optimize();
				int Nsol = model.get(GRB_IntAttr_SolCount);
				double val_fo = model.get(GRB_DoubleAttr_ObjVal)/M_T;
				M_fo_formulation1 = val_fo;
				M_fo_formulation2= model.get(GRB_DoubleAttr_ObjBound)/M_T;
				int ndistintos = M_Integer_Sol_Distintas.size();
				for (auto i = 0; i < Nsol; i++)
				{
					model.set(GRB_IntParam_SolutionNumber, i);
...
//					cout << model.get(GRB_DoubleAttr_PoolObjVal) << " " << val   << "";
			...
			M_Time_Modelo3 = time;
			if (!M_Challenge_Mode) printf("\nTiempo lleva Gurobi %.2f Tiempo total %.2f\n", time, Tiempo_Total_Gurobi);
			if (M_Tipo_Formulation==0 && (kiter<30) && (kiter == Max_Objectives - 1) && time < (Tiempo_Total_Gurobi-10  ))
			{
				M_Time_Populate = Tiempo_Total_Gurobi - time;
				Max_Objectives++;
...
//					if (kiter%3!=0)
					...
					vals_obj.add((M_Interventions[i].Get_RiskQuantile(l) * (Alpha)) +
					    ((1 - Alpha) * M_Interventions[i].Get_RiskC(l)));
				
					if (kiter > 0 && (M_Best_I_T[i] == l) &&
					    (M_Interventions[i].Get_RiskC(l) >M_Interventions[i].Get_Min_Risk()) && 
						(M_Interventions[i].Get_RiskQuantile(l) > M_Interventions[i].Get_Min_RiskQ()) &&
					    get_random(0, 10) < 4)
						x_ij[i][l].setLB(1);
					else
						x_ij[i][l].setLB(0);
