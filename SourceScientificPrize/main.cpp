...
//	Problema.~Maintenance();
	...
	{ //GRASP Inicial
		int TiempoGrasp = min((maxSeconds / 10),30)+tmp_first2; //		TiempoGrasp = 10; //PARA EL GRASP //PAra hacerlo con fopenmp
//		TiempoGrasp = 300;
...
#pragma omp for schedule(dynamic, 1)  nowait 
		...
		//Solve with gurobi
		ftime( &time_final);
		tmp_first2 = ((double)((time_final.time - time_ini.time) * 1000 +time_final.millitm - time_ini.millitm)) /1000;
...
//			Iter_Grasp+=Problemas[cont].Get_Iter();
		...
		int Total_Time_Intensification = 0;
		Total_Time_Intensification = Total_Time /4;
...
#pragma omp for schedule(static, 1)   nowait //#pragma omp for schedule(dynamic, 1)
			...
			Total_Time_Intensification = Total_Time_Intensification / 8;
			for (int i = 0; i < 8; i++)
			{
...
#pragma omp for schedule(static, 1)   nowait //#pragma omp for schedule(dynamic, 1)
					for (int Global_Iter = 0; Global_Iter < nThreads; Global_Iter++)
					{
						...
						if (tmp_first3 < maxSeconds)
						{
							...
							Problemas[Global_Iter].SetTotalTime(Total_Time_Intensification);
							Problemas[Global_Iter].SetIntensification(true);
